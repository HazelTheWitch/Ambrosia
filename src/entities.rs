...
pub fn positioned<'w>(entity: Result<&'w mut Entity, ECSError>, x: i32, y: i32, priority: u8) -> Result<&'w mut Entity, ECSError> {
    entity?.insert_component(Position::new(x, y, priority))
}

pub fn renderable<'w>(entity: Result<&'w mut Entity, ECSError>, x: i32, y: i32, priority: u8, character: char, fg: (u8, u8, u8), bg: (u8, u8, u8)) -> Result<&'w mut Entity, ECSError> {
    positioned(entity, x, y, priority)?
        .insert_component(SingleGlyphRenderer::new(rltk::to_cp437(character), RGB::named(fg), RGB::named(bg)))
}
